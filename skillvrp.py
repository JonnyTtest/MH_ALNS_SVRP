#!/usr/bin/env python3
"""Prize-collecting Skill-VRP solver (single-file submission).

Usage:
    python3 skillvrp.py <path to instance> <timeout (s)>

Runs a greedy multi-start construction followed by ALNS (adaptive large
neighborhood search) within the time limit and prints the best found
solution in checker format to stdout.

This file bundles instance_reader.py, solution.py, initial_solution.py and
alns.py together with the benchmark configuration from test_run.ipynb
...
# ===========================================================================
# Entry point: benchmark configuration from test_run.ipynb
# ("Benchmark über alle Instanzen")
# ===========================================================================

# alns.py referenziert die Greedy-Insertion unter diesem Namen.
greedy_insert_customer = insert_customer

# Sicherheitsmarge: reserviert fuer Einlesen/Greedy-Rest und die Ausgabe,
# damit das Programm sicher innerhalb des Zeitlimits terminiert.
OUTPUT_MARGIN_SECONDS = 1.0


def make_operators(inst):
    """Frisches Operator-Set mit den Benchmark-Defaults.

    Operatoren sind zustandsbehaftet (weight, total_uses, segment_*), daher
    braucht jeder run_alns-Aufruf ein unabhaengiges Set.
    """
    ...
    # Iterationszahl einbricht.
    max_remove = max(5, min(100, int(round(n * 0.25))))

    bf = (0.05, 0.15)   # Basis-Destroy
    rf = (0.05, 0.20)   # Related/Shaw
    eul = 40            # Repair-Kandidatenpool
    mds = 0.0
...
def solve(inst, time_limit: float) -> Solution:
    ...
    if remaining <= 0.5:
        # Zu wenig Zeit fuer ALNS; die Greedy-Loesung ist bereits feasible.
    ...
    destroy_operators, repair_operators = make_operators(inst)
    ...
    # Nie schlechter als der Greedy-Start (best ist monoton, aber die
    # Garantie ist billig).
    if best.objective < start_solution.objective:
        return start_solution
    return best
...
def main() -> None:
    ...
    try:
        solution = solve(inst, args.timeout)
    except Exception:
        # Der leere Plan ist immer feasible (Profit 0): im Fehlerfall trotzdem
        # eine gueltige Loesung ausgeben.
        import traceback
        traceback.print_exc(file=sys.stderr)
        solution = Solution.empty(inst)

    solution.print_for_checker()
